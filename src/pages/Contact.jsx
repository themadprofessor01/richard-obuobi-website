  ...
  return (
    <>
      ...
      <section className="bg-deep-navy pt-28 pb-16 md:pt-36 md:pb-20">
        <div className="max-w-7xl mx-auto px-5 md:px-10">
          <div ref={heroRef} className="reveal">
            ...
            <p className="font-inter text-ivory/60 text-lg max-w-xl">
      ...
      <section className="bg-white py-16 md:py-24">
        <div className="max-w-6xl mx-auto px-5 md:px-10 grid grid-cols-1 lg:grid-cols-3 gap-12">
          ...
          <aside className="space-y-8">
            ...
            <div className="bg-ivory rounded-2xl p-6 border border-gold/20">
              <Shield size={20} className="text-gold mb-3" />
            ...
            <div className="flex gap-1 bg-gray-100 rounded-lg p-1 mb-8 w-fit">
              {[['booking', 'Book a Consultation'], ['message', 'Send a Message']].map(([key, label]) => (
                <button
                  ...
                  className={`font-manrope text-sm font-semibold px-5 py-2.5 rounded-md transition-all ${
                    tab === key
              ...
              <>
                ...
                {tab === 'booking' && (
                  <div>
                    ...
                    <div className="flex items-center gap-3 mb-8">
                      {[1, 2].map(n => (
                        <div key={n} className="flex items-center gap-2">
                          <div className={`w-8 h-8 rounded-full flex items-center justify-center font-manrope text-sm font-bold transition-all ${
                          ...
                          {n < 2 && <span className="text-gray-300 ml-2">→</span>}
                        </div>
                      ))}
                    </div>
                    ...
                    {step === 1 && (
                      <div>
                        <h3 className="font-playfair text-xl text-charcoal mb-2">Preferred Time Slot</h3>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-6">
                          {timeSlots.map(t => (
                            <button
                              key={t}
                              onClick={() => setSlot(t)}
                              className={`p-4 rounded-xl border text-left font-inter text-sm transition-all ${
                                slot === t
                                  ? 'border-gold bg-gold/5 text-charcoal font-semibold'
                                  : 'border-gray-200 text-muted-gray hover:border-navy'
                              }`}
                            >
                              {t}
                            </button>
                        ...
                        <h3 className="font-playfair text-xl text-charcoal mb-4">Purpose of Consultation</h3>
                        ...
                        <button
                          ...
                          onClick={() => setStep(2)}
                          className="font-manrope font-bold bg-navy text-ivory px-8 py-3.5 rounded hover:bg-deep-navy transition-colors disabled:opacity-40"
                        >
                          Continue →
                        </button>
                      </div>
                    ...
                    {step === 2 && (
                      <form onSubmit={handleBookingSubmit} className="space-y-5">
                        <div className="bg-ivory rounded-xl p-4 border border-gold/20 mb-6">
                        ...
                        <div className="flex gap-3">
                          <button type="button" onClick={() => setStep(1)} className="font-manrope text-sm text-muted-gray border border-gray-200 px-5 py-3 rounded hover:border-navy transition-colors">
                            ← Back
                          </button>
                          <button type="submit" disabled={status === 'sending'} className="font-manrope font-bold bg-navy text-ivory px-8 py-3 rounded hover:bg-deep-navy transition-colors disabled:opacity-60">
                ...
                {tab === 'message' && (
                  <form onSubmit={handleMessageSubmit} className="space-y-5">
                    ...
                    <div>
                      ...
                      <textarea required rows={6} value={formData.message} onChange={e => setFormData(p => ({ ...p, message: e.target.value }))} className="w-full border border-gray-200 rounded-lg px-4 py-3 font-inter text-sm focus:outline-none focus:border-gold resize-none" placeholder="How can Richard help you?" />
                    </div>
                    <button type="submit" disabled={status === 'sending'} className="font-manrope font-bold bg-navy text-ivory px-8 py-3.5 rounded hover:bg-deep-navy transition-colors disabled:opacity-60">
